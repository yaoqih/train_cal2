...
def touches_remote(request: ResourceRequest) -> bool:
    return any(line in legacy.REMOTE_INTERACTION_LINES for line in request.touched_lines)
...
def depot_put_line_count(request: ResourceRequest) -> int:
    return sum(1 for line in request.put_lines if line in legacy.DEPOT_LINES)
...
def is_remote_release_to_cun4(envelope: CandidateEnvelope, request: ResourceRequest) -> bool:
    return (
        ...
        and request.source_line in legacy.REMOTE_INTERACTION_LINES
...
def is_remote_outbound_session_release(envelope: CandidateEnvelope, request: ResourceRequest) -> bool:
    ...
    return (
        ...
        and len(puts) == 1
        and puts[0].line == "存4线"
        and all(step.line in legacy.REMOTE_INTERACTION_LINES for step in gets)
    )
